...
function GeminiKneeAssessment({ review, state }: { review: IllustrativeCase["geminiVisualReview"]; state: IllustrativeCase["geminiVisualState"] }) {
  if (!review) {
    return <section className="rounded-[1.6rem] border border-[#EDE3E5] bg-white p-6 shadow-[0_18px_45px_-38px_rgba(92,49,61,0.38)]"><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[#A6556A]">Gemini knee assessment</p><h3 className="mt-2 text-xl font-extrabold text-[#3E3036]">Image review pending</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-[#806C73]">{state?.message ?? "Upload a readable knee MRI image to gather visual OA findings, anatomy descriptors, and available image-derived values."}</p></section>;
  }
  ...
  return <section className="rounded-[1.6rem] border border-[#EDE3E5] bg-white p-6 shadow-[0_18px_45px_-38px_rgba(92,49,61,0.38)]"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[#A6556A]">Gemini knee assessment</p><h3 className="mt-2 text-2xl font-extrabold text-[#3E3036]">OA review and measurements</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-[#806C73]">Image-derived OA findings, anatomy observations, and available scale-guided values for clinician review.</p></div><span className="w-fit rounded-full bg-[#F8E7EB] px-3 py-1.5 text-xs font-extrabold text-[#934F60]">{review.model}</span></div>
    ...
    <div className="mt-6 rounded-xl border border-[#EEE3E5] bg-[#FFFCFC] p-4"><p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#9C858C]">Planning support</p><p className="mt-2 text-sm font-extrabold capitalize text-[#493940]">{planningLabel}</p><p className="mt-2 text-xs leading-5 text-[#806C73]">{review.implantPlanning.rationale}</p></div><p className="mt-6 border-t border-[#F0E7E9] pt-4 text-xs leading-5 text-[#806C73]">{review.reviewNote} Image-derived values and OA findings require clinician confirmation.</p></section>;
}
...
export default function KneeAnalysis() {
  ...
  return <KneeCoAppShell eyebrow="Clinical case" title="Knee Analysis"><div className="mx-auto max-w-7xl"><button type="button" onClick={() => setLocation("/cases")} className="mb-5 inline-flex items-center text-sm font-extrabold text-[#8A5563] transition hover:text-[#733963]"><ArrowLeft className="mr-2 h-4 w-4" />Back to Case Overview</button>
    <section className="rounded-[1.75rem] border border-[#EFE4E6] bg-white p-7 shadow-[0_18px_45px_-35px_rgba(92,49,61,0.35)] sm:p-9"><div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[0.17em] text-[#A6556A]">AI-assisted review</p><h2 className="font-kneeco-display mt-3 text-4xl tracking-[-0.04em] text-[#352C30]">Knee Analysis</h2><p className="mt-3 text-sm leading-6 text-[#7A676E]">Case {record.id} · {record.patientLabel} · {record.kneeSide} knee</p></div><div className="flex flex-wrap items-center gap-3"><span className="rounded-full bg-[#F8E7EB] px-3 py-2 text-xs font-extrabold text-[#934F60]">{record.geminiVisualReview ? "Gemini findings available" : "Awaiting image review"}</span><Button type="button" onClick={removeCase} disabled={deleteCase.isPending} variant="outline" className="rounded-xl border-[#EBCFD5] bg-white text-[#A6556A] hover:bg-[#FFF2F4] hover:text-[#8A3E50]"><Trash2 className="mr-2 h-4 w-4" />Delete Case</Button><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#F8E7EB] text-[#A6556A]"><ShieldCheck className="h-5 w-5" /></span></div></div><div className="mt-8 flex w-full overflow-x-auto rounded-xl border border-[#EDE2E5] bg-[#FFFAFB] p-1.5">{tabs.map((item) => <button type="button" key={item.id} onClick={() => setTab(item.id)} className={`min-w-[150px] flex-1 rounded-lg px-4 py-2.5 text-sm font-extrabold transition ${tab === item.id ? "bg-[#733963] text-white shadow-sm" : "text-[#7E686F] hover:bg-white hover:text-[#533F47]"}`}>{item.label}</button>)}</div></section>
    {tab === "analysis" && <section className="mt-7"><GeminiKneeAssessment review={record.geminiVisualReview} state={record.geminiVisualState} /></section>}
