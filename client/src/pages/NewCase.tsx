...
export default function NewCase() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    if (!scan) {
      setError("Attach a DICOM or NIfTI knee MRI before creating the case.");
      return;
    }
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <form onSubmit={submit} className="grid gap-7 lg:grid-cols-[1fr_340px]">
          <div className="space-y-7">
            ...
            <section className="rounded-[1.6rem] border border-[#EDE3E5] bg-white p-6 shadow-[0_18px_45px_-38px_rgba(92,49,61,0.38)] sm:p-8">
              <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[#A6556A]">MRI attachment</p><h3 className="mt-2 text-xl font-extrabold text-[#3E3036]">Attach the knee study</h3><p className="mt-2 text-sm leading-6 text-[#7D6A71]">KneeCo accepts DICOM or NIfTI MRI input for a measurement-capable workflow. Maximum intake size: 20 MB.</p>
              <label className="mt-6 flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-[#D9B8C0] bg-[#FFFAFB] px-6 text-center transition hover:border-[#C97C8D] hover:bg-[#FFF7F8]"><input type="file" accept=".dcm,.nii,.nii.gz,application/dicom,application/octet-stream" onChange={selectScan} className="sr-only" /><FileUp className="h-7 w-7 text-[#A6556A]" /><span className="mt-3 text-sm font-extrabold text-[#513D44]">Choose knee MRI file</span><span className="mt-1 text-xs text-[#8A747B]">DICOM (.dcm) or NIfTI (.nii/.nii.gz)</span></label>
          ...
          <aside className="h-fit rounded-[1.6rem] border border-[#E5D8DC] bg-white p-6 shadow-[0_18px_45px_-38px_rgba(92,49,61,0.38)] lg:sticky lg:top-24"><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[#A6556A]">Case readiness</p><h3 className="mt-3 text-xl font-extrabold text-[#3E3036]">Ready when complete.</h3><div className="mt-6 space-y-4 text-sm text-[#735F67]"><p className="flex gap-3"><CheckCircle2 className={`h-5 w-5 shrink-0 ${form.patientId && form.patientName ? "text-[#A6556A]" : "text-[#D9C8CC]"}`} />Patient identity</p><p className="flex gap-3"><CheckCircle2 className={`h-5 w-5 shrink-0 ${form.age && form.kneeSide !== "unknown" ? "text-[#A6556A]" : "text-[#D9C8CC]"}`} />Clinical context</p><p className="flex gap-3"><CheckCircle2 className={`h-5 w-5 shrink-0 ${scan ? "text-[#A6556A]" : "text-[#D9C8CC]"}`} />MRI attachment</p></div><div className="mt-7 flex gap-2 rounded-xl border border-[#F0E5E7] bg-[#FFFAFB] p-4 text-xs leading-5 text-[#79666D]"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-[#A6556A]" /><p>The case will be recorded with <strong>analysis pending validation</strong> until the MRI analysis service is validated on a compatible study.</p></div>{error && <p role="alert" className="mt-5 rounded-xl bg-[#FFF0F1] p-3 text-sm font-semibold text-[#A04658]">{error}</p>}<Button disabled={createCase.isPending} type="submit" className="mt-6 h-12 w-full rounded-xl bg-[#C97C8D] text-sm font-extrabold text-white hover:bg-[#A6556A] disabled:opacity-70">{createCase.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating case</> : <>Create Case<ArrowRight className="ml-2 h-4 w-4" /></>}</Button><p className="mt-4 text-center text-[11px] font-semibold uppercase tracking-[0.12em] text-[#A18A91]">Decision support only</p></aside>
        </form>
      </div>
    </KneeCoAppShell>
  );
